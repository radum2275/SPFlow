...
class RSPMNInitialTemplate:
    ...
    def build_initial_template(self, data):
        ...
        self.spmn_structure_two_time_steps = copy.deepcopy(spmn_structure_two_time_steps)

        logging.debug(f'length_of_time_slice {self.two_time_step_params.length_of_each_time_slice}')
        ...
        scope_top_interface_parent = [scope for scope, var in
                                      ...
                                      and scope >= self.two_time_step_params.length_of_each_time_slice - 1]

        logging.debug(f'scope_top_interface_parent {scope_top_interface_parent}')

        top_network, template_network = self.get_top_and_interface_networks(spmn_structure_two_time_steps,
                                                                            scope_top_interface_parent)
        self.top_network = top_network
        self.template_network = template_network

        return self.spmn_structure_two_time_steps, top_network, template_network

    def get_top_and_interface_networks(self, spmn_structure_two_time_steps, scope_top_interface_parent):
        """
        :param spmn_structure_two_time_steps:
        :param scope_top_interface_parent: scope of node that connects last variable in time slice zero and
        rest of the variables in time slice one.
        :return: top and template network sliced from input spmn for two time steps
        """
        ...
        # perform bfs
        seen, queue = set([spmn_structure_two_time_steps]), collections.deque([spmn_structure_two_time_steps])
        interface_num = 0
        while queue:
            node = queue.popleft()
            if not isinstance(node, Leaf):
                ...
                for child in node.children:
                    ...
                    if isinstance(node, Product) and node.scope == scope_top_interface_parent:
                        ...
                        if set(child.scope).issubset(scope_time_slice_1):
                            ...
                            logging.debug(f'time_step_1_children {time_step_1_children}')
                ...
                if len(time_step_1_children) > 0 and isinstance(node, Product) \
                    ...
                    # create latent interface node for corresponding interface node
                    ...
                    logging.debug(f'latent interface child scope {latent_interface_child.scope}')
    ...
    def make_interface_node(children_list):
        """
        :param children_list: time step one children of time step zero parent node that is one level above last leaf
        ...
        if len(children_list) > 1:
            # more than one child of interface parent prod node belong to time step 1
    ...
    def make_template_network(self, interface_nodes):
        ...
        :return: template network with interface switch as root of interface nodes and interface nodes connected
        at bottom for next time step
        """

        # interface switch scope is equal to its child interface node's scope, all of its children have same scope

        template_network = InterfaceSwitch(children=interface_nodes)
        template_network = self.attach_interface_nodes_to_template_network_at_bottom(template_network)
        assign_ids(template_network)

        return template_network

    def attach_interface_nodes_to_template_network_at_bottom(self, interface_switch):
        """
        :param interface_switch: network with interface switch as root and interface nodes as its children
        :return: full template network with interface nodes connected at bottom for next time step
        ...
        while queue:
            ...
            if not isinstance(node, Leaf):
                ...
                # get node at level before leaf nodes. This is connected to next time step interface nodes
